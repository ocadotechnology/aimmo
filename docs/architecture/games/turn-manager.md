...
The Turn Manger is a Daemon that runs in parallel with [Worker Manager](worker-manager). While the purpose of the worker manager is to supervise the game room, the turn manager handles the actual simulation of the game. It is the "game loop".

The Turn Manager runs as follows:
* ask each avatar for its decision
* update the environment using the [World Map](world-map)
* run a callback at the end of each turn (the callback registered in the [service](Flusk-Microservice) is sending the world updates through the socket)
...
The avatars are asked for their decisions by calling the [Game API](game-api). The environment is updated by calling [World Maps's methods](world-map), which are provided by the [State Provider](state-provider). For more insight on how different simulation objects work take a look at [Game Objects wiki](Game-objects).