...
class TestDamagePickupsAndEffects(TestCase):
    def setUp(self):
        ...
        _avatar_spawn_cell = self.game.game_state.world_map.get_cell(
            Location(0, 0))
    ...
    def test_damage_boost_pickup_can_be_picked_up_default(self):
        ...
        self.assertEqual(
            len(self.game.avatar_manager.get_avatar(1).effects), 1)
        damage_boost_effect = self.game.avatar_manager.get_avatar(
            1).effects.pop()
        self.assertTrue(isinstance(damage_boost_effect,
                                   pickup_created.effects[0]))
    ...
    def test_damage_boost_increases_attack_strength_with_default_integer(self):
        ...
        self.assertTrue(self.cell.avatar.attack_strength,
