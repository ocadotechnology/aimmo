...
class TestTurnManager(unittest.TestCase):
    ...
    def construct_turn_manager(self, *avatars):
        ...
        self.world_state = WorldState(WorldMap(self.construct_default_level()), self.avatar_manager)
        self.turn_manager = TurnManager(self.world_state)

    def test_run_turn(self):
        avatar = AvatarRunner(Location(0, 0), dummy, 0, self.construct_default_avatar_appearance())
    ...
    def test_run_several_turns_and_avatars(self):
        avatar1 = AvatarRunner(Location(0, 0), dummy, 0, self.construct_default_avatar_appearance())
        avatar2 = AvatarRunner(Location(0, 1), dummy, 1, self.construct_default_avatar_appearance())
