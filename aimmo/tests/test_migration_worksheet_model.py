...
def test_worksheet_has_correct_fields(migrator):
    ...
    assert model._meta.get_field("era").get_internal_type() == "PositiveSmallIntegerField"
...
def test_worksheet_has_new_fields_added(migrator):
    ...
    assert Worksheet._meta.get_field("short_description").get_internal_type() == "TextField"
    assert Worksheet._meta.get_field("image_path").get_internal_type() == "CharField"
    assert Worksheet._meta.get_field("active_image_path").get_internal_type() == "CharField"
    assert Worksheet._meta.get_field("thumbnail_image_path").get_internal_type() == "CharField"
    assert Worksheet._meta.get_field("thumbnail_text").get_internal_type() == "CharField"
...
def test_worksheet_has_new_pdf_name_fields(migrator):
    ...
    new_state = migrator.apply_tested_migration(("aimmo", "0021_add_pdf_names_to_worksheet"))
    Worksheet = new_state.apps.get_model("aimmo", "Worksheet")

    assert Worksheet._meta.get_field("teacher_pdf_name").get_internal_type() == "CharField"
    assert Worksheet._meta.get_field("student_pdf_name").get_internal_type() == "CharField"
