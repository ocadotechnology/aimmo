...
def test_create_game_secret(game_secret_manager, game_id, caplog: pytest.LogCaptureFixture):
    ...
    game_secret_manager.create_game_secret(
        game_id=game_id, game_name=game_name, token=token
    )
    game_secret_manager.api.create_namespaced_secret.assert_called_with(
...
def test_delete_game_secret(game_secret_manager: GameSecretManager, game_id: int):
    ...
    type(
        game_secret_manager.api.list_namespaced_secret.return_value
    ).items = PropertyMock(return_value=[mock_secret])

    game_secret_manager.delete_game_secret(game_id=game_id)
    game_secret_manager.api.delete_namespaced_secret.assert_called_with(
